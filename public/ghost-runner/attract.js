...
// The attract tile plays the gameplay background (gameBgVideo below). The
// ~54 MB start-screen video (background animation 1.mp4) is only used by the
// fullscreen game, which warms it into the HTTP cache at booth boot - so don't
// load it here, or every home-screen load downloads 54 MB it never draws.
...
// Draw the game background video. This build loops on the home tile all day, so
// a stalling video must never blank the canvas: #0a0a1a is rgb(10,10,26), still
// blue-dominant, and cutting to it on every re-buffer is the intermittent blue
// flicker on the home screen. Hold the last decoded frame instead - a stall then
// reads as a briefly frozen background rather than a flash.
let attractLastFrame = null;
let attractFrameTick = 0;

function drawGameBackground() {
  attractFrameTick++;
  if (gameBgVideo.readyState >= 2 && gameBgVideo.videoWidth > 0) {
    if (gameBgVideo.paused) gameBgVideo.play();
    ctx.drawImage(gameBgVideo, 0, 0, canvas.width, canvas.height);

    // Refresh the held frame every ~10 frames; a full-canvas snapshot every
    // frame would double the per-frame draw cost for no visible benefit.
    if (attractFrameTick % 10 === 0) {
      if (!attractLastFrame) attractLastFrame = document.createElement("canvas");
      if (attractLastFrame.width !== canvas.width || attractLastFrame.height !== canvas.height) {
        attractLastFrame.width = canvas.width;
        attractLastFrame.height = canvas.height;
      }
      attractLastFrame.getContext("2d").drawImage(gameBgVideo, 0, 0, attractLastFrame.width, attractLastFrame.height);
    }
    return;
  }

  if (attractLastFrame && attractLastFrame.width) {
    ctx.drawImage(attractLastFrame, 0, 0, canvas.width, canvas.height);
    return;
  }

  // Nothing decoded yet - the fallback is all we have.
  ctx.fillStyle = "#0a0a1a";
  ctx.fillRect(0, 0, canvas.width, canvas.height);
}
