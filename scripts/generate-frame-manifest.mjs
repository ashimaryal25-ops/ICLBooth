// Generates src/data/frame-themes.json from the frame PNGs in public/frames.
// Photo windows are detected from the alpha channel (large fully-transparent
// regions, 4-connected components). Types + loader live in src/data/frame-themes.ts;
...
// theme -> label, per photo-count source filenames.
const THEMES = [
  ...
  ["football", "Football", { 2: ["football pic2.png", "backend-football-2pic.png"], 3: ["football pic3.png", "backend-football-3pic.png"], 4: ["football pic4.png", "backend-football-4pic.png"] }],
  ["lacrosse", "Lacrosse", { 2: ["lacrosse pic2.png", "backend-lacrosse-2pic.png"], 3: ["lacrosse pic3.png", "backend-lacrosse-3pic.png"], 4: ["lacrosse pic 4.png", "backend-lacrosse-4pic.png"] }],
...
async function main() {
  ...
  const dest = path.join(ROOT, "src/data/frame-themes.json");
  fs.writeFileSync(dest, json, "utf-8");
  process.stdout.write(
    `wrote ${dest}: ${entries.length} variants, ${problems.length} problems\n`,
  );
}
